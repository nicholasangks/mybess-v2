...
function ConfirmationModal({ message, onConfirmAction, onCloseAction }: { message: string, onConfirmAction: () => void, onCloseAction: () => void }) {
    return (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center backdrop-blur-sm z-50">
            <div className="bg-white dark:bg-background-d p-5 rounded-lg w-[300px] text-center">
                <p className="mb-4 text-base text-muted-foreground dark:text-muted-foreground-d">{message}</p>
                <div className="flex justify-between gap-3">
                    <button className="flex items-center justify-center w-full h-[2.2rem] rounded-md bg-muted dark:bg-muted-d" onClick={onCloseAction}>Cancel</button>
            ...
            <div className="max-w-[750px] mt-[5rem] mx-auto">
                ...
                <div className="flex items-center">
                    ...
                    <div className="flex items-center cursor-pointer" onClick={() => {
                        ...
                        <div className={`flex items-center justify-center w-[4rem] aspect-square mr-2 rounded-full border-[2px] border-white ${controlData?.onOff.toLowerCase() === 'on' ? 'border-primary' : 'border-muted dark:border-muted-d'}`}>
                            <div className={`flex items-center justify-center w-[80%] aspect-square rounded-full ${controlData?.onOff.toLowerCase() === 'on' ? 'bg-primary/30 blur-md' : 'bg-muted dark:bg-muted-d'}`}></div>
                            <LuPower className="absolute text-xl" />
                        </div>
                        <div className={`font-medium ${controlData?.onOff.toLowerCase() === 'on' ? 'text-primary' : 'text-muted-foreground dark:text-muted-foreground'}`}>{controlData?.onOff}</div>
                    </div>
                </div>
                    ...
                    <div className="border-r border-border dark:border-border-d pr-10">
                        ...
                        <div className="grid grid-cols-2 gap-5 mt-4">
                            {['remote-mode', 'local-mode'].map(mode => (
                                <div key={mode} onClick={() => handleSubmit("remoteCtrl", mode)} className={`flex items-center justify-center w-full aspect-square rounded-full border-[2px] cursor-pointer ${controlData?.remoteCtrl.toLowerCase() === mode ? 'border-primary' : 'border-muted dark:border-muted-d'}`}>
                                    <div className={`flex items-center justify-center w-[90%] aspect-square rounded-full ${controlData?.remoteCtrl.toLowerCase() === mode ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground dark:text-muted-foreground-d'}`}>{mode === 'remote-mode' ? 'Remote' : 'Local'}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="pl-10">
                        ...
                        <div className="grid grid-cols-2 gap-5 mt-4">
                            {['pq-mode', 'vf-mode'].map(mode => (
                                <div key={mode} onClick={() => handleSubmit("gridModeCtrl", mode)} className={`flex items-center justify-center w-full aspect-square rounded-full border-[2px] cursor-pointer ${controlData?.gridModeCtrl.toLowerCase() === mode ? 'border-primary' : 'border-muted dark:border-muted-d'}`}>
                                    <div className={`flex items-center justify-center w-[90%] aspect-square rounded-full capitalize ${controlData?.gridModeCtrl.toLowerCase() === mode ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground dark:text-muted-foreground-d'}`}>{mode}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {[
                        ...
                        <div key={label}>
                            ...
                            <div className="mt-2 p-1 rounded-lg border border-border dark:border-border-d">
                                ...
                                />
                                <div
                                    className="flex items-center justify-center mt-2 h-[2.1rem] bg-muted dark:bg-muted-d rounded-md text-center cursor-pointer"
